...
$sql = "SELECT id, fname, lname, age, password FROM patient WHERE username = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("s", $myusername);
$stmt->execute();
$stmt->bind_result($id, $fname, $lname, $age, $password);
$stmt->fetch();
$stmt->close();
...
if (isset($_POST["submit"])) {
  ...
  if($errors == 0)
  {
    ...
    $sql = "UPDATE patient SET fname=?, lname=?, age=?, password=? WHERE id=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssi", $ufname, $ulname, $uage, $upassword, $id);
    $result = $stmt->execute();
    $stmt->close();
